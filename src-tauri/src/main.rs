...
/// Return the per-launch auth token generated by the API server process.
/// The backend writes the token to `~/.config/omnilauncher/server-token` at
/// startup; the Tauri shell reads it here and passes it to the frontend so
/// every HTTP request to the API server carries the correct token.
#[tauri::command]
fn get_server_token() -> String {
    std::fs::read_to_string(server_token_path())
        .unwrap_or_default()
        .trim()
        .to_string()
}
...
fn main() {
    ...
    if server_mode {
        ...
        // shell's `get_server_token` command can read it back.
        let auth_token = server::generate_auth_token();
