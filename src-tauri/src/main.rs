...
/// Path where the split backend writes its per-launch auth token so the Tauri
/// shell can read it back via the `get_split_token` command.
fn split_token_path() -> std::path::PathBuf {
    let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
    let mut p = std::path::PathBuf::from(home);
    p.push(".config");
    p.push("omnilauncher");
...
/// Return the per-launch auth token generated by the split backend process.
/// The backend writes the token to `~/.config/omnilauncher/split-token` at
/// startup; the Tauri shell reads it here and passes it to the frontend so
/// every HTTP request to the split backend carries the correct token.
#[tauri::command]
fn get_split_token() -> String {
    std::fs::read_to_string(split_token_path())
        .unwrap_or_default()
        .trim()
        .to_string()
}
...
async fn slash_preview(
    ...
    let pm = state.plugin_manager.lock().await;
    Ok(omnilauncher_lib::split_server::slash_preview_backend(&query, &pm).await)
}
...
fn main() {
    ...
    if split_backend_only {
        ...
        // Generate a per-launch auth token and write it to disk so the Tauri
        // shell's `get_split_token` command can read it back.
        let auth_token = split_server::generate_auth_token();
        if let Err(e) = std::fs::write(split_token_path(), auth_token.as_bytes()) {
            log::warn!("failed to persist split auth token: {e}");
        } else {
            log::info!("split auth token written to {}", split_token_path().display());
        }
