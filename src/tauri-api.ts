...
export async function invoke<T = unknown>(cmd: string, args?: Record<string, unknown>): Promise<T> {
  if (!isTauri()) {
    ...
    if (cmd === 'search') {
      return [
        { id: '1', title: 'Calculator', subtitle: 'App', score: 1, action_type: 'open', action_data: 'calc', icon: '🧮' },
        { id: '2', title: 'Notepad', subtitle: 'App', score: 0.8, action_type: 'open', action_data: 'notepad', icon: '📝' },
      ] as T
    }
    if (cmd === 'ai_query') {
      return {
