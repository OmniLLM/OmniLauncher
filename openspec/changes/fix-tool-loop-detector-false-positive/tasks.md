...
- [ ] 1.1 In `src-tauri/src/ai/router.rs`, extract a pure helper
...
- [ ] 1.2 Add `#[cfg(test)] mod loop_detector_tests` with three tests:
...
- [ ] 1.3 Run `cargo test --lib --manifest-path src-tauri/Cargo.toml
      loop_detector_tests` and confirm all three pass.

## 2. Wire the helper into the agentic loop

- [ ] 2.1 Replace the inline fingerprint comparison at
...
- [ ] 2.2 Build the richer loop-trip `final_content`:
...
- [ ] 2.3 Add a `tracing::debug!` log line when the detector trips,
      logging the redacted tool name + result snippet.
- [ ] 2.4 `cargo build --manifest-path src-tauri/Cargo.toml` succeeds.

## 3. Settings field and wiring

- [ ] 3.1 In `src-tauri/src/settings.rs`, add field
...
- [ ] 3.2 Update existing settings tests:
      ...
      new field at `true`.
- [ ] 3.3 Add `test_preserves_custom_ai_loop_detector_field` that
      round-trips a JSON blob with `ai_loop_detector_enabled: false`.
- [ ] 3.4 Thread the field from `AppSettings` through to the agentic
...
- [ ] 3.6 `cargo test --lib --manifest-path src-tauri/Cargo.toml settings`
      passes.

## 4. Frontend toggle

- [ ] 4.1 In `src/types/app.ts`, add `ai_loop_detector_enabled: boolean`
      to the `AppSettings` interface and to the in-memory seed object
      used by `SettingsWindow.tsx`.
- [ ] 4.2 In `src/components/SettingsWindow.tsx`, add a checkbox to the
...
- [ ] 4.3 `npm test -- --run` passes (no logic-test impact expected, but
      run as a smoke).

## 5. Validate and commit

- [ ] 5.1 `openspec validate fix-tool-loop-detector-false-positive`
      passes.
- [ ] 5.2 `cargo test --manifest-path src-tauri/Cargo.toml` passes
      end-to-end.
- [ ] 5.3 Stage the change directory and the source / frontend edits,
